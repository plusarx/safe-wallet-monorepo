...
export interface OrderParams {
  ...
  totalAmount: string
  amountMode?: 'fixed' | 'percentage' // 'fixed' = fixed amount per client, 'percentage' = % of client balance
...
function buildSlippageAnalysis(
  ...
  const priceImpactPercent = expectedPrice > 0 ? ((fillPrice - expectedPrice) / expectedPrice) * 100 : 0
...
export function useOrderEngine() {
  ...
  const executeBatchTrade = useCallback(
    async (
      clientAddresses: string[],
      tokenIn: { address: string; decimals: number },
      tokenOut: { address: string; decimals: number },
  ...
  const executeMarketOrder = useCallback(
    async (params: OrderParams): Promise<boolean> => {
  ...
  const executeTWAPOrder = useCallback(
    async (params: OrderParams): Promise<boolean> => {
      ...
      const totalExpectedQuote = await getQuote(
        tokenIn.address,
        tokenOut.address,
        totalAmount,
        tokenIn.decimals,
      ...
      for (let i = 0; i < slices; i++) {
        ...
        const realizedSlip =
          expectedSoFar > 0 ? calculateRealizedSlippage(expectedSoFar.toString(), totalActualOut.toString()) : 0
  ...
  const executeSmartMarketOrder = useCallback(
    async (params: OrderParams): Promise<boolean> => {
      ...
      const realizedSlip =
        expectedForExecuted > 0
          ? calculateRealizedSlippage(expectedForExecuted.toString(), totalActualOut.toString())
          : 0
      const priceImpact =
        totalActualOut > 0 ? calculatePriceImpact(executedVolume.toString(), totalActualOut.toString(), spotPrice) : 0

      // Build slippage analysis for A/B testing
      const slippageAnalysis =
        totalActualOut > 0
          ? buildSlippageAnalysis(
  ...
  const executeSmartTWAPOrder = useCallback(
    async (params: OrderParams): Promise<boolean> => {
      const {
        clientAddresses,
        tokenIn,
      ...
      for (let sliceIdx = 0; sliceIdx < slices; sliceIdx++) {
        ...
        const expectedForProcessed =
          (parseFloat(totalExpectedQuote) / (slices * chunks)) * (totalChunksProcessed - failedChunks)
        const realizedSlip =
          expectedForProcessed > 0
            ? calculateRealizedSlippage(expectedForProcessed.toString(), totalActualOut.toString())
            : 0
  ...
  const executeOrder = useCallback(
    async (params: OrderParams): Promise<OrderState> => {
      setIsExecuting(true)
      setError(null)
  ...
  const getSlippageAnalytics = useCallback(() => {
    ...
    const calculateMetrics = (orderGroup: OrderState[]) => {
      ...
      const slippages = orderGroup.filter((o) => o.realizedSlippage !== undefined).map((o) => o.realizedSlippage!)

      const priceImpacts = orderGroup.filter((o) => o.priceImpact !== undefined).map((o) => o.priceImpact!)
      ...
      return {
        ...
        avgRealizedSlippage: slippages.length > 0 ? slippages.reduce((a, b) => a + b, 0) / slippages.length : 0,
        avgPriceImpact: priceImpacts.length > 0 ? priceImpacts.reduce((a, b) => a + b, 0) / priceImpacts.length : 0,
        ...
        avgSlippageSetting:
          slippageSettings.length > 0 ? slippageSettings.reduce((a, b) => a + b, 0) / slippageSettings.length : 0,
      }
    }
    ...
    return {
      ...
      rawOrders: filledOrders.map((o) => ({
        ...
        isConcurrent:
          o.slippageAnalysis?.isConcurrent ?? (o.params.orderType === 'market' || o.params.orderType === 'twap'),
        // Quantity
