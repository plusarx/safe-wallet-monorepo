              ...
              <Box display="flex" justifyContent="space-between" alignItems="center" mb={1}>
                <Box display="flex" alignItems="center" gap={1}>
                  ...
                  {selectedClients.length > 0 && (
                    <Chip label={`${selectedClients.length} selected`} size="small" color="primary" />
                  )}
                </Box>
                <FormControlLabel
                  control={
            ...
            <Grid container spacing={2} alignItems="flex-end">
              <Grid item xs={6} sm={3}>
                <FormControl fullWidth size="small">
                  ...
                  <Select value={tokenIn} label="Sell" onChange={(e) => setTokenIn(e.target.value)}>
                    {tokenList.map((t) => (
              ...
              <Grid item xs={6} sm={3}>
                <FormControl fullWidth size="small">
                  ...
                  <Select value={tokenOut} label="Buy" onChange={(e) => setTokenOut(e.target.value)}>
                    {tokenList
                      .filter((t) => t !== tokenIn)
                      .map((t) => (
                        <MenuItem key={t} value={t}>
                    ...
                    <TableRow key={order.id}>
                      <TableCell>
                        <Chip
                          ...
                          size="small"
                          color={
                            order.type === 'stop-market' ? 'error' : order.type === 'stop-limit' ? 'warning' : 'primary'
                          }
                          variant="outlined"
                        />
                      </TableCell>
                      ...
                      <TableCell align="right">
                        <Typography variant="body2">{parseFloat(order.triggerPrice).toFixed(4)}</Typography>
                        {order.limitPrice && (
