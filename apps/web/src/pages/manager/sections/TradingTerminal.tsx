...
export default function TradingTerminal() {
            ...
            for (const addr of clientAddresses) {
                ...
                if (delegation) {
                    ...
                    if (tradingModule && tokens) {
                        await Promise.all(Object.values(tokens).map(async (t) => {
                            if (!t.address || t.address === '0na') return
                            try {
                                const contract = new Contract(t.address, ['function allowance(address,address) view returns (uint256)'], provider)
                                const allowance = await contract.allowance(addr, tradingModule)
                                if (allowance > 0n) approvals.push(t.symbol)
                            } catch (e) { console.warn('Allowance check failed', t.symbol) }
                        }))
                    }
    ...
    const toggleClient = (address: string) => {
        setClients(prev => prev.map(c =>
            c.address === address ? { ...c, selected: !c.selected } : c
        ))
    }
                        ...
                        {clients.length === 0 ? (
                            ...
                            <>
                                ...
                                <FormControlLabel
                                    control={
                                        <Checkbox
                                            checked={clients.filter(c => walletTypeFilter === 'all' || c.walletType === walletTypeFilter).every(c => c.selected)}
                                            indeterminate={selectedClients.length > 0 && selectedClients.length < clients.filter(c => walletTypeFilter === 'all' || c.walletType === walletTypeFilter).length}
                                            onChange={toggleSelectAll}
                                        />
                                    }
                                    label={<Typography variant="body2">Select All ({clients.filter(c => walletTypeFilter === 'all' || c.walletType === walletTypeFilter).length})</Typography>}
                                />
                                <Divider sx={{ my: 1 }} />
                ...
                <Grid item xs={12} md={5}>
                    <Paper sx={{ p: 3 }}>
                        ...
                        <Paper sx={{ p: 2, mt: -0.5, bgcolor: 'rgba(255,255,255,0.05)' }}>
                            ...
                            <Box display="flex" gap={2} alignItems="center">
                                <Typography variant="h5" sx={{ flex: 1, color: isQuoting ? 'text.secondary' : 'text.primary' }}>
                                    {isQuoting ? 'Fetching...' : (quoteAmountOut ? '~' + parseFloat(quoteAmountOut).toLocaleString(undefined, { maximumFractionDigits: 9 }) : '0.0')}
                                </Typography>
                                <FormControl size="small" sx={{ minWidth: 100 }}>
                                    <Select
                        ...
                        <Box display="flex" justifyContent="space-between" alignItems="center" mt={2} mb={1}>
                            <Typography variant="caption" color="text.secondary">Slippage Tolerance</Typography>
                            <Box display="flex" gap={0.5}>
                                {['0.5', '1.0', '2.0'].map((val) => (
                                    <Chip
                                        key={val}
                        ...
                        {amountIn && parseFloat(amountIn) > 0 && (
                            <Box sx={{ bgcolor: 'rgba(255,255,255,0.03)', borderRadius: 1, p: 1.5, mt: 2 }}>
                                <Box display="flex" justifyContent="space-between" mb={0.5}>
                                    <Typography variant="caption" color="text.secondary">Min. Receive</Typography>
                                    <Typography variant="caption">
                                        {(parseFloat(quoteAmountOut || '0') * (1 - parseFloat(slippage) / 100)).toLocaleString(undefined, { maximumFractionDigits: 9 })} {tokenOut.symbol}
                                    </Typography>
                                </Box>
                                <Box display="flex" justifyContent="space-between">
                                    <Typography variant="caption" color="text.secondary">Platform Fee (0.1%)</Typography>
                                    <Typography variant="caption">
                        ...
                        >
                            {isTrading ? 'Executing...' :
                                selectedClients.length === 0 ? 'Select Client(s)' :
                                    selectedClients.length > 1
